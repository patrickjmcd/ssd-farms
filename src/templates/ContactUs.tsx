...
export const ContactUs = ({emailServiceId, emailTemplateId, emailPublicKey}: ContactUsProps) => {
    ...
    return (
        <Formik initialValues={{name: '', message: '', email: '', phone: ''}}
                ...
                <form onSubmit={handleSubmit} className={"flex flex-col space-y-4"}>
                    ...
                    <div>
                        <Input
                            id={'phone'}
                            name={'phone'}
                            label={'Phone Number (if you would prefer to be contacted by phone)'}
                            type={'tel'}
                            errors={errors}
                            touched={touched}
                        />
                    ...
                    {!sendError && !sendSuccess && (<div className={'self-center'}>
                        <button type="submit" disabled={isSubmitting}
                                className={"text-sky-200 bg-sky-700 px-8 py-3 rounded-lg text-xl"}>Submit
                        </button>
                    </div>)}
